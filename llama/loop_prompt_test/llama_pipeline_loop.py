...
def generate_next_sentence(conversation, conversational_pipeline, tokenizer):
...
def ask_for_sentence_text_gen():
    sentence = input("your answer : ")
    return "Child : " + sentence
...
def main():
    ...
    if PIPE_TYPE == "text-generation":
        ...
        for i in range(CONV_LENGTH):
            print("Loading...")
            time_0 = time.time()
            result = generate_next_sentence(prompt, conversational_pipeline, tokenizer)
            time_1 = time.time()
            print("["+str(round(time_1 - time_0, 3)) + "s]")
            agent_sentence = ""
            for r in result:
                agent_sentence += r['generated_text']
                print(r['generated_text'])
            sentence = ask_for_sentence_text_gen()
            prompt = agent_sentence + sentence
    ...
    if PIPE_TYPE == "conversational":
        ...
        for i in range(CONV_LENGTH):
            print("Loading...")
            time_0 = time.time()
            conversation = generate_next_sentence(conversation, conversational_pipeline, tokenizer)
            time_1 = time.time()
