...
class LlamaCppMemoryIncrementalInterruption:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_pattern_size = max([len(p) for p in self.role_token_text_patterns])
    ...
    def remove_stop_patterns(self, sentence):
        ...
        for i, pattern in enumerate(self.stop_token_text_patterns):
            if pattern == last_chunck_string[-len(pattern) :]:
    ...
    def is_stop_token(self, token, word):
        """
        ...
        is_stopping_text = word in self.stop_token_text
    ...
    def is_role_pattern(self, sentence):
        ...
        if self.max_pattern_size < len(sentence):
            return False, None
    ...
    def generate_next_sentence(
        ...
        for token in self.model.generate(
            ...
            is_stop_token = self.is_stop_token(token, word)
            ...
            if self.interruption:
                ...
                is_punctuation = self.is_punctuation(word)
                subprocess(
                    word,
                    is_punctuation,
                    role_pattern,
