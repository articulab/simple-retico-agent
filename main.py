...
def main_demo():
    """The `main_demo` function creates and runs a dialog system that is able
...
def test_body():
    """Testing if an ActiveMQ retico module (AMQWriterOpening) could trigger
    SARA's oppening animation."""
...
def test_body_2():
    """ADDITION : testing if sound and further dialogue animations could be
    triggered by ActiveMQ retico modules (using fake BEAT and TTS modules).
    Testing SARA's body activation with a retico module simulating BEAT output
    (AMQWriter) and another simulating TTS output (fakeTTSSARA). The fakeTTS
    module plays the speech.mp3 file, that is not the file matching the
    gesture.

    SARA's body is moving every time the ASR is recognizing voice, and
    sending a message to the fake BEAT module. -> Which means that it is
    possible to control SARA's body entirely with a real BEAT-like
    module implemented in retico (and with retico modules sending data
    to the unity agent through an MQ like ActiveMQ).
    """
...
def test_body_3():
    """ADDITION : using real BEAT module instead of a fake BEAT outputting the
    same message each trigger.

    Testing SARA's body activation using a retico pipeline and BEAT for
    the gesture generation. The retico pipeline is MIC - VAD - ASR, with
    3 additional modules : - TextAnswertoBEATBridge : a module creating
    a bridge between the ASR and BEAT - AMQWriter : a module that sends
    the IUs to ActiveMQ (to communicate with BEAT and SARA body). module
    simulating BEAT output (AMQWriter) - fakeTTSSARA : a module that
    simulates SARA's TTS (fakeTTSSARA). The fakeTTS module plays the
    speech.mp3 file, that is not the file matching the gesture, but it
    is useful because the gestures are not played without receiving
    activeMQ message that says that the agent is talking.

    SARA's body is moving with the message received by BEAT module. ->
    Which means that it is possible to control SARA's body entirely with
    a real BEAT-like module implemented in retico (and with retico
    modules sending data to the unity agent through an MQ like
    ActiveMQ).
    """
...
def test_body_4():
    """Testing SARA's body activation using a retico pipeline and BEAT for the
    gesture generation.

    The retico pipeline is MIC - VAD - ASR, with 3 additional modules :
    - TextAnswertoBEATBridge : a module creating a bridge between the
    ASR and BEAT - AMQWriter : a module that sends the IUs to ActiveMQ
    (to communicate with BEAT and SARA body). module simulating BEAT
    output (AMQWriter) - fakeTTSSARA : a module that simulates SARA's
    TTS (fakeTTSSARA). The fakeTTS module plays the speech.mp3 file,
    that is not the file matching the gesture, but it is useful because
    the gestures are not played without receiving activeMQ message that
    says that the agent is talking.
    ...
    TODO: - integrate a NLG module to create an textual answer -
    integrate a real TTS retico module in the pipeline, to synthetise
    voice from the textual answer - integrate a speaker module in the
    pipeline - make the speaker module send ActiveMQ messages to SARA's
    body when it plays the sound to verify that it can be synchronized.
    """
